...
## 2. Planned Implementation Steps
...
### B. Dedicated Express.js Backend (Render/Railway/Custom)
- Create a `server` directory at the project root for an Express.js app.
...
- Update the root `package.json` with a script (e.g., via `concurrently`) to launch both Next.js (`:3000`) and Express (`:3001`) simultaneously.

## 3. Planned UX & Codebase Improvements
